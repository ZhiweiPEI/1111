...
function findAll() {
    ...
    let token = tokenHead +" "+ access_token;
    $.ajax({
        url: 'http://localhost:8080/back/news/findAll',//地址：访问后台的地址
...
function edit(id) {
    ...
    xadmin.open('编辑','news-edit.html',750,400);
}

function editStatus(id) {
    sessionStorage.setItem("currentId",id);
    xadmin.open('编辑','news-editStatus.html',600,200);
...
function getCount(){
    ...
    let token = tokenHead +" "+ access_token;
    $.ajax({
        url: 'http://localhost:8080/back/news/count',//地址：访问后台的地址
        data: getSearchData(),//前端传递给后台的数据
        type: 'post',
        async: false,
        dataType: 'json',//是后端传递到前端的数据格式
