...
function member_stop(obj,id){
    ...
    layer.confirm('确认要'+ status +'吗？',function(index){
        ...
        $.ajax({
            url: 'http://localhost:8080/back/role/updateEnable',
            data: {id: id},
            type: 'post',
            dataType: 'json',
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
        ...
        let token = tokenHead +" "+ access_token;
        $.ajax({
            url: 'http://localhost:8080/back/role/delete',
            data: {id: id},
            type: 'post',
            dataType: 'json',
            beforeSend: function(request) {
...
function findAll() {
    ...
    let token = tokenHead +" "+ access_token;
    $.ajax({
        url: 'http://localhost:8080/back/role/findAll',//地址：访问后台的地址
...
function setList(data)  {
    ...
    for (var i = 0; i < data.length; i++) {
        html += '<tr>\n' +
            ...
            '                            <td>' + setUndefinedToNull(data[i].creatTime) + '</td>\n' +
            '                            <td class="td-status">\n' +
...
function edit(id) {
    ...
    xadmin.open('编辑','role-edit.html',600,400);
}
...
function getCount(){
    ...
    let token = tokenHead +" "+ access_token;
    $.ajax({
        url: 'http://localhost:8080/back/role/getCount',//地址：访问后台的地址
        data: getSearchData(),//前端传递给后台的数据
        type: 'post',
        async: false,
