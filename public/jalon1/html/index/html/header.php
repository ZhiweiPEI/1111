<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>header</title>
    <link rel="stylesheet" href="../../../css/css_index/base.css">
    <link rel="stylesheet" href="../../../css/css_index/header.css">
</head>
<body>
    <div class="header">

        <div class="nav nav-menu">
            <div class="nav-con nav-con-right fl">
                <ul>
                    <img class="fl"src="../../../img/index_img/imtLogo.png" height="35.99" class="face">
<<<<<<< HEAD:public/jalon1/html/index/html/header.html
                    <li><a href="http://localhost:8080/jalon1/html/login.php">LOG IN</a></li>
                    <li><a href="http://localhost:8080/jalon1/html/logup.php">LOG UP</a></li>
                    <li><a href="http://localhost:8080/jalon1/html/user/index.php">User Info</a></li>
                    <li><a href="">Message</a></li>
=======
                    <li><a href="../../login.php" target="a1">LOG IN</a></li>
                    <li><a href="../../logup.php" target="a1">LOG UP</a></li>
                    <li><a href="">User Info</a></li>
                    <li><a href="../../user/favorite.php" target="a1">Favorite</a></li>
>>>>>>> 2759137e98ff72568d0a07f8e1a5bf84a1ca3fc2:public/jalon1/html/index/html/header.php
                    <li><a href="">History</a></li>
                </ul>
            </div>
        </div>
        <div class="nav-mask nav"></div>
        <div class="banner">
            <div>
                <img src="../../../img/index_img/headerBackground.png" alt="" width="100%" height="180px">
            </div>

            <div class="search ">
                <div class="search-left">
                    <div class="search-img fl"></div>
                    <div class="search-div-a">
                        <a href="">Rank</a>
                    </div>
                </div>
                <div class="search-right">
                    <form class="" action="" >
                        <input type="text" width: 200px; placeholder="Enter your desired search here">
                        <button type="submit" ><a id="submit" >search</a></button>
                    </form>
                </div>
            </div>
        </div>
        <div class="nav-menu-bottom">
            <ul class="">
                <li>
                    <a href="">
                        <div class="nav-menu-bottom-img"><img src="../../../img/index_img/home.png"></div>
                        <div class="nav-name">Home</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">Music</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">Film</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">Anime</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">manga</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">series</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">dance</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">games</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">technology</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">life</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="num-wrap"><span>99+</span></div>
                        <div class="nav-name">fashion</div>
                    </a>
                </li>


                <li>
                    <a href="">
                        <div class="nav-menu-bottom-img"><img src="../../../img/nav/square.png"></div>
                        <div class="nav-name">HOT</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="nav-menu-bottom-img"><img src="../../../img/nav/live.png"></div>
                        <div class="nav-name">LIVE</div>
                    </a>
                </li>
                <li>
                    <a href="">
                        <div class="nav-menu-bottom-img"><img src="../../../img/nav/xiaoheiwu.png"></div>
                        <div class="nav-name">NEWS</div>
                    </a>
                </li>
                <img  class="fr" src="../../../img/nav/bysj.gif" width="69px" height="40px"alt="">
            </ul>
        </div>
    </div>
<script type="text/javascript">

</script>
</body>
</html>
